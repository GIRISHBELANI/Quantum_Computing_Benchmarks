...
def run2 (min_input_qubits=1, max_input_qubits=16, skip_qubits=1,
    ...
    for num_input_qubits in range(min_input_qubits, max_input_qubits + 1, skip_qubits):
        ...
        for num_clock_qubits in range(min_clock_qubits, max_clock_qubits+1, skip_qubits):      
            ...
            if use_best_widths:
                expected_input_qubits = int((num_qubits - 1) / 3)
                expected_clock_qubits = (num_qubits - 1 - 2 * expected_input_qubits)
                
                if verbose:
                    print(f"num_qubits: {num_qubits}, num_input_qubits: {num_input_qubits}, num_clock_qubits: {num_clock_qubits}")
                    print(f"expected_input_qubits: {expected_input_qubits}, expected_clock_qubits: {expected_clock_qubits}")
